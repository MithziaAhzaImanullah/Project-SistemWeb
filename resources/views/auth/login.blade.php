...
<body class="bg-slate-50 text-slate-800 font-sans antialiased min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
    ...
    <div class="sm:mx-auto w-full max-w-md z-10">
        ...
        <h2 class="text-center text-2xl font-extrabold tracking-tight text-slate-900">Selamat datang kembali</h2>
        <p class="mt-2 text-center text-sm text-slate-500">Masuk untuk mengelola wisata favorit dan melihat history pencarianmu.</p>
    </div>
        ...
        <div class="bg-white py-8 px-4 shadow-xl border border-slate-100 rounded-3xl sm:px-10">
            ...
            <form class="space-y-6" action="{{ route('login') }}" method="POST">
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <a href="{{ route('password.request') }}" class="text-sm font-semibold text-emerald-600 hover:text-emerald-700 transition">Lupa password?</a>
                        @endif
                    </div>
                    <div class="relative rounded-xl bg-slate-50 border border-slate-200 focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-100 transition">
                ...
                <div class="flex items-center">
                    <input id="remember_me" name="remember" type="checkbox" class="h-4 w-4 text-emerald-600 border-slate-300 rounded">
                    <label for="remember_me" class="ml-2 block text-sm text-slate-600 font-medium select-none">Ingat saya di perangkat ini</label>
                </div>
            ...
            <div class="mt-6 pt-6 border-t border-slate-100 text-center">
                <p class="text-sm text-slate-500">
                    Belum punya akun?
                    <a href="{{ route('register') }}" class="font-bold text-emerald-600 hover:text-emerald-700 transition">Daftar Akun Baru</a>
                </p>
            </div>
        </div>
